...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            statStr = iStr.format(titleStr, typeStr, iouStr, areaStr, maxDetsStr, '%.3f'%(float(mean_s)))
            print statStr 
            return mean_s, statStr

        if not self.eval:
            raise Exception('Please run accumulate() first')
        self.stats = np.zeros((12,))
        self.statsStr = ['' for i in range(12)]
        self.stats[0], self.statsStr[0] = _summarize(1)
        self.stats[1], self.statsStr[1] = _summarize(1,iouThr=.5)
        self.stats[2], self.statsStr[2] = _summarize(1,iouThr=.75)
        self.stats[3], self.statsStr[3] = _summarize(1,areaRng='small')
        self.stats[4], self.statsStr[4] = _summarize(1,areaRng='medium')
        self.stats[5], self.statsStr[5] = _summarize(1,areaRng='large')
        self.stats[6], self.statsStr[6] = _summarize(0,maxDets=1)
        self.stats[7], self.statsStr[7] = _summarize(0,maxDets=10)
        self.stats[8], self.statsStr[8] = _summarize(0,maxDets=100)
        self.stats[9], self.statsStr[9]  = _summarize(0,areaRng='small')
        self.stats[10], self.statsStr[10] = _summarize(0,areaRng='medium')
        self.stats[11], self.statsStr[11] = _summarize(0,areaRng='large')
