...
def load_coco(image_set, root_path, shuffle=False):
    ...
    imdbs = []
    for s in image_set:
        imdbs.append(coco(s, root_path, shuffle, is_train=True))
    if len(imdbs) > 1:
...
def load_pascal(image_set, root_path, shuffle=False):
    """
    wrapper function for loading pascal voc dataset
    ...
    year = [y.split('_')[1].strip() for y in image_set]
    image_set = [y.split('_')[0].strip() for y in image_set]
    assert image_set, "No image_set specified"
