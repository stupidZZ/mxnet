...
namespace mshadow {
  namespace cuda {
    ...
    __global__ void DeformablePSROIPoolForwardKernel(
      ...
      CUDA_KERNEL_LOOP(index, count) {
        ...
        if (pad_mode) {
          top_data[index] = ele_count == 0 ? static_cast<DType>(0) : sum / (sample_per_part * sample_per_part);
        } else {
          top_data[index] = ele_count == 0 ? static_cast<DType>(0) : sum / ele_count;
        }
      }
    }
    ...
    inline void DeformablePSROIPoolForward(const Tensor<gpu, 4, DType> &out,
      ...
      DeformablePSROIPoolForwardKernel<DType> << <mxnet::op::mxnet_op::cuda_get_num_blocks(count),
        ...
        group_size, part_size, num_classes, channels_each_class, pad_mode, top_data);
    ...
    __global__ void DeformablePSROIPoolBackwardAccKernel(
      ...
      CUDA_KERNEL_LOOP(index, count) {
        ...
        if (pad_mode) top_count = sample_per_part * sample_per_part;

        DType diff_val = top_diff[index] / static_cast<DType>(top_count);
        for (int ih = 0; ih < sample_per_part; ih++) {
          for (int iw = 0; iw < sample_per_part; iw++) {
            DType w = wstart + iw*sub_bin_size_w;
            DType h = hstart + ih*sub_bin_size_h;
    ...
    inline void DeformablePSROIPoolBackwardAcc(const Tensor<gpu, 4, DType> &in_grad,
      ...
      DeformablePSROIPoolBackwardAccKernel<DType> << <mxnet::op::mxnet_op::cuda_get_num_blocks(count),
        ...
        group_size, part_size, num_classes, channels_each_class, pad_mode);
      DeformablePSROIPOOLING_CUDA_CHECK(cudaPeekAtLastError());
    }
  ...
  inline void DeformablePSROIPoolForward(const Tensor<gpu, 4, DType> &out,
    ...
    const float trans_std,
    const bool pad_mode) {
    cuda::DeformablePSROIPoolForward(out, data, bbox, trans, no_trans, spatial_scale,
      output_dim, group_size, pooled_size, part_size, sample_per_part, trans_std, pad_mode);
  }
  ...
  inline void DeformablePSROIPoolBackwardAcc(const Tensor<gpu, 4, DType> &in_grad,
    ...
    const float trans_std,
    const bool pad_mode) {
    cuda::DeformablePSROIPoolBackwardAcc(in_grad, trans_grad, out_grad, data, bbox, trans, no_trans,
      spatial_scale, output_dim, group_size, pooled_size, part_size, sample_per_part, trans_std, pad_mode);
  }
